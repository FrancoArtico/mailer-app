...
def send(to, subject, content):
    ...
    from_email = Email(current_app.config['FROM_EMAIL']) # debe usarse mail con extension de la empresa
    to_email = To(to)
...
def create():
    if request.method == 'POST':
        ...
        if not subject:
            errors.append('Asunto del correo es obligatorio')
        if not content:
